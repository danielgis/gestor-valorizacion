...
define([
  ...
  function queryFeature(queryObject, map){
    console.log("asdasdasdasdasdasdasdad...");
    /************************
  ...
  function selectFeatures(map, layer, queryArray){
    ...
    //@danielgis
    map.getLayer(layer.layerInfo.id).setDefinitionExpression(query.where);
    const districtLayer = map.getLayer("limite_nacionalV2_8724");

    districtLayer.queryExtent(query)
    .then(ext => {
      map.setExtent(ext.extent);
    })
